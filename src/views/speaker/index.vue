  ...
  <div class="app-container">
    <div style="margin-bottom:20px;">
      <el-row>
        ...
        <el-col :span="6">
          <router-link to="add">
            <el-button type="success"> 添加嘉宾</el-button>
          </router-link>
        </el-col>
      </el-row>
    </div>
    ...
    <el-table :data="list" v-loading.body="listLoading" element-loading-text="Loading" border fit highlight-current-row>
      <el-table-column align="center" label='ID' width="95">
        <template slot-scope="scope">
          {{scope.row.speaker_id}}
        </template>
      </el-table-column>
      <el-table-column label="嘉宾名称" align="center">
        <template slot-scope="scope">
          {{scope.row.speaker_name}}
        </template>
      </el-table-column>
      <el-table-column label="嘉宾身份" align="center">
        <template slot-scope="scope">
          {{scope.row.speaker_identity}}
        </template>
      </el-table-column>
      <el-table-column class-name="status-col" label="嘉宾状态" width="100" align="center" >
        <template slot-scope="scope">
          <el-tag :type="scope.row.speaker_state | statusFilter">
            <span v-if="scope.row.speaker_state == '1'" >上线 </span>
            <span v-if="scope.row.speaker_state == '-1'" >下线 </span>
          </el-tag>
        </template>
      </el-table-column>
      ...
      <el-table-column align="center" prop="created_at" label="编辑" >
        <template slot-scope="scope">
          <el-tooltip class="item" effect="dark" content="编辑信息" placement="top">
            <router-link :to="{name:'SpeakerDetail',query:{speaker_id:scope.row.speaker_id}}">
          ...
          <el-tooltip class="item" effect="dark" content="更改状态" placement="top">
            <span v-if="scope.row.speaker_state == '1'">
